$(document).ready(function() {
	...
	$('.sub-sub-menu li:first-child').addClass('firstChild'); //Fix for IE borders only
	 ...
	 if($this.hasClass('head')) {
           $('.accordion li > a span').empty().append('<img src="va_files/2012/images/left-nav/closed.png" alt="Closed Arrow" >');
           $('.sub-menu li > a span').empty().append('<img src="va_files/2012/images/left-nav/sub-closed.png" alt="Closed Arrow" >');
           $('.sub-sub-menu li > a span').empty().append('<img src="va_files/2012/images/left-nav/sub-closed.png" alt="Closed Arrow"  >');
	 };
    });
   ...
   $('.accordion li.head > a').on('click', function(e) {
		...
		if($parent.parent().hasClass('accordion')){
			$siblings.find('span').empty().append('<img src="va_files/2012/images/left-nav/closed.png" alt="Closed Arrow" >');
			$siblings.find('ul').find('span').empty().append('<img src="va_files/2012/images/left-nav/sub-closed.png" alt="Closed Arrow" >');
		}
		else {
			$siblings.find('span').empty().append('<img src="va_files/2012/images/left-nav/sub-closed.png" alt="Closed Arrow" >');
		};
        ...
        if($parent.hasClass('active')) {
			if($parent.parent().hasClass('accordion')){
				$(this).find('span').first().empty().append('<img src="va_files/2012/images/left-nav/open.png" alt="Open Arrow" >');
			}
			else {
               $(this).find('span').first().empty().append('<img src="va_files/2012/images/left-nav/sub-open.png" alt="Open Arrow" >');
			};
        }
        else {
            if($parent.parent().hasClass('accordion')){
				$(this).find('span').first().empty().append('<img src="va_files/2012/images/left-nav/closed.png" alt="Closed Arrow" >');
	    }
            else {
               $(this).find('span').first().empty().append('<img src="va_files/2012/images/left-nav/sub-closed.png" alt="Closed Arrow">');
	    };
...
;
	...
	if($pageType != "homepage") {
		...
		$('#leftNav a').each(function() {
			if ($(this).attr('href').match(urlPagei)) {
				$match = "true";
		...
		$('#leftNav a').each(function() {
			if ($match == "false") {
				...
				if ($(this).attr('href').match(urlIndexi)) {
	        			$(this).parents('li.head').children('a').trigger('click');
					return false;
				};
			};
